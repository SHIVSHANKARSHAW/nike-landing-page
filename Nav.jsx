    ...
    <>
      <header
        className={`padding-x py-4 fixed z-50 transition-all duration-300 bg-white ${
          atTop ? 'w-full' : 'w-[86%] md:w-[95%] ml-8 mt-5 rounded-lg shadow-lg'
      ...
      >
        <nav className='flex justify-between items-center max-container'>
          ...
          <div className='hidden max-lg:block'>
            <img src={hamburger} alt='hamburger icon' width={25} height={25} onClick={toggleMobileNav} />
          </div>
        </nav>
      </header>
      {isModalOpen && <Modal onClose={() => setIsModalOpen(false)} onLogin={handleLogin} />}
      {isMobileNavOpen && (
        <div className='fixed top-0 left-0 w-full h-full bg-white z-40 flex flex-col items-center justify-center'>
          ...
          <div className='flex gap-2 text-lg leading-normal font-medium font-montserrat mt-8'>
            {isLoggedIn ? (
              <button onClick={() => { handleLogout(); setIsMobileNavOpen(false); }} className="hover:scale-110 font-medium">Logout</button>
            ) : (
              <>
                <button onClick={() => { setIsModalOpen(true); setIsMobileNavOpen(false); }} className="hover:scale-110 font-medium outline-none">Login</button>
                <span>/</span>
                <button onClick={() => { setIsModalOpen(true); setIsMobileNavOpen(false); }} className="hover:scale-110 font-medium outline-none">SignUp</button>
              </>
            )}
          </div>
        </div>
